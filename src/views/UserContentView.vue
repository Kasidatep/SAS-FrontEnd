...
onMounted(async () => {
    ...
    pages.value = await getPages(mode.value, 0, size, announcementCategory.value)
...
const pageChange = async (page) => {
    pages.value = await getPages(mode.value, page, size, announcementCategory.value)
    announcements.value = await pages.value.content
}
...
const switchMode = async () => {
    mode.value = (mode.value == "active") ? "close" : "active"
    pages.value = await getPages(mode.value, 0, size, announcementCategory.value)
    announcements.value = await pages.value.content
        ...
        <div class="w-[95%] sm:w-[90%] pt-14 min-h-[72.5vh]">
            ...
            <div class="w-full flex justify-between pb-8  items-center flex-col lg:flex-row">
                ...
                <div class="flex items-center flex-col justify-center md:flex-row gap-4 pt-4 lg:pt-0 md:pb-2 lg:pb-0">
                    <div @click="switchMode" v-if="mode == 'active'"
                    ...
                    <div @click="switchMode" v-if="mode == 'close'"
            ...
            <div v-for="(announcement, index) in announcements " v-if="announcements?.length !== 0">
                <UserAnnouncementItems :announcement="announcement" :index="getIndex(index)" class="ann-item" :mode="mode"/>
            </div>
            ...
            <div v-if="pages?.totalElements >= 5" class="pt-12 flex">
                ...
                <button class="px-5 py-1.5 bg-[#e9f4fe] cursor-pointer text-lg "
                    :class="`ann-page-${index}`, index == pages?.page ? 'btnfocus' : ''"
